...
#	echo bwa mem -M -R '<read group info>' -p tauschii-chrom6 $x-R1.fq $x-R2.fq : $x.sam
	bwa mem -t 30 -M -R '@RG\tID:tauschiiALL\tSM:sample1\tPL:illumina\tLB:lib1\tPU:unit1' 131201_BGI_wheat_A_D_combined_pseudochr.fasta ~/$x/TM-R1_val_1.fq.gz ~/$x/TM-R2_val_2.fq.gz > TM.sam
	echo SortSam
 java -Xmx10g -Djava.io.tmpdir=`pwd`/tmp -jar ~/picard-tools-1.110/SortSam.jar INPUT= TM.sam OUTPUT= TM.picard.sorted.bam SORT_ORDER=coordinate TMP_DIR=`pwd`/tmp
	echo MarkDuplicates
 java -Xmx10g -Djava.io.tmpdir=`pwd`/tmp -jar ~/picard-tools-1.110/MarkDuplicates.jar INPUT= TM.picard.sorted.bam OUTPUT= TM.picard.dedup.bam METRICS_FILE=metricsTM-Martin.txt TMP_DIR=`pwd`/tmp
	echo IndexBam - output *dedup.bai
 java -Xmx10g -jar ~/picard-tools-1.110/BuildBamIndex.jar INPUT= TM.picard.dedup.bam&
