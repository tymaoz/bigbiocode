...
for x in ${roots[@]}; do
	...
	bwa mem -t 18 -M -R '@RG\tID:S2ALL\tSM:sample1\tPL:illumina\tLB:lib1\tPU:unit1' virtualpoly6  fastq/$x-R1.fastq.gz fastq/$x-R2.fastq.gz | samtools view -Sub - | samtools sort - $x.sorted
#	echo SortSam
...
#  samtools view -Su "$x".sam | samtools sort - "$x".sorted
        echo Piling up ${x}
  samtools mpileup -A -uf Chrom6-gatk-cons-virtual-polyploid.fa $x.sorted.bam 
