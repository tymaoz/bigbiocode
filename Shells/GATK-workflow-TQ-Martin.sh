...
#	echo bwa mem -M -R '<read group info>' -p tauschii-chrom6 $x-R1.fq $x-R2.fq : $x.sam
	bwa mem -t 20 -M -R '@RG\tID:tauschiiALL\tSM:sample1\tPL:illumina\tLB:lib1\tPU:unit1' 131201_BGI_wheat_A_D_combined_pseudochr.fasta ~/$x/TQ-all-R1_val_1.fq.gz ~/$x/TQ-all-R2_val_2.fq.gz > $x.sam
	echo SortSam
 java -Xmx10g -Djava.io.tmpdir=`pwd`/tmp -jar ~/picard-tools-1.110/SortSam.jar INPUT= $x.sam OUTPUT= $x.picard.sorted.bam SORT_ORDER=coordinate TMP_DIR=`pwd`/tmp
	echo MarkDuplicates
 java -Xmx10g -Djava.io.tmpdir=`pwd`/tmp -jar ~/picard-tools-1.110/MarkDuplicates.jar INPUT= $x.picard.sorted.bam OUTPUT= $x.picard.dedup.bam METRICS_FILE=metricsTauschii-Martin.txt TMP_DIR=`pwd`/tmp
