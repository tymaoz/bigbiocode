...
roots=(
TQ-all-aligned_trimmed-reads_tauschii6
)
...
# java -Djava.io.tmpdir=`pwd`/tmp -jar picard-tools-1.110/BuildBamIndex.jar INPUT=$x.picard.dedup.bam&
 ...
 #	java -Xmx2g -jar ~/picard-tools-1.110/AddOrReplaceReadGroups.jar INPUT=$x.sorted.bam OUTPUT=$x.withgroups.sorted.bam SORT_ORDER=coordinate RGID=TMC RGLB=TMC2013 RGPL=illumina RGPU=201310 RGSM=TM-C RGCN=INCPM RGPI=100
	...
	java -Xmx5g -jar GATK/GenomeAnalysisTK.jar -T RealignerTargetCreator -R ~/WheatD/tauschii-chrom6.2.fa -I $x.picard.dedup.bam -o $x.target_intervals.list
	echo Realign at Targets near Indels
	java -Xmx5g -jar GATK/GenomeAnalysisTK.jar -T IndelRealigner -R ~/WheatD/tauschii-chrom6.2.fa -I $x.picard.dedup.bam -targetIntervals $x.target_intervals.list -o $x.realignedBam.bam
	echo Call Variants with GATK
	java -Xmx5g -jar GATK/GenomeAnalysisTK.jar -T UnifiedGenotyper -R ~/WheatD/tauschii-chrom6.2.fa -I $x.realignedBam.bam -L 20 -ploidy 2 -glm BOTH -stand_call_conf 30 -stand_emit_conf 10 -o $x.GATKvariants.vcf
	echo Alternate Reference Maker
	java -Xmx5g -jar GATK/GenomeAnalysisTK.jar -T FastaAlternateReferenceMaker -l ERROR -R ~/WheatD/tauschii-chrom6.2.fa -o $x.gatk-cons.fa --variant $x.GATKvariants.vcf&
########SAMTOOLS*PIPELINE################
